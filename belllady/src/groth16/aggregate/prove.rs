...
pub fn aggregate_proofs_and_instances<E: Engine + std::fmt::Debug>(
    ...
    for j in 0..poly_f.len() {
        ...
        for i in 0..poly_f[j].len() {
            poly_eval.add_assign( & mul!( poly_f[j][i].clone(), &r_pow));
            r_pow.mul_assign( &r );
        }
        f_eval.push(poly_eval.clone() );

        f_eval_proof.push( create_kzg_opening_for_instance::<E>(
            &srs.g_alpha_powers,
...
fn create_kzg_opening_for_instance<E: Engine>(
    srs_powers: &Vec<E::G1Affine>, // h^alpha^i
    ...
    try_par! {
        // Check F^x^(n - d) exists i.e. that F is bounded
        let pi_open = inner_product::multiexponentiation::<E::G1Affine>(
            &srs_powers[..quotient_polynomial_coeffs.len()], &quotient_polynomial_coeffs)
        };
